...
const SectionHeader = ({ icon, title, subtitle }) => (
  <div style={{ marginBottom: 28 }}>
    <div style={{ fontSize: 13, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--primary-light)', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end' }}>
          ...
          <motion.button
            ...
            style={{
              ...
              cursor: 'pointer', fontFamily: 'inherit', opacity: (!word.trim() || isPending) ? 0.6 : 1,
            }}
          >
            {isPending ? '🔍…' : '🔍 Search'}
          </motion.button>
            ...
            <Card>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    <h2 style={{ fontSize: 28, fontWeight: 900, letterSpacing: '-0.03em' }}>{result.word}</h2>
                    {result.phonetic && (
                      <span style={{ fontSize: 15, color: 'var(--text3)', fontFamily: 'var(--font-mono)' }}>/{result.phonetic}/</span>
                    )}
                    ...
                    {result.audioUrl && (
                      <motion.button
                        ...
                        style={{ width: 32, height: 32, borderRadius: 9, background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(99,102,241,0.25)', color: 'var(--primary-light)', cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                {result.meanings.map((m, mi) => (
                  <div key={mi}>
                    <div style={{ fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--primary-light)', marginBottom: 12 }}>
                      {m.partOfSpeech}
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {m.definitions.map((d, di) => (
                        <div key={di} style={{ paddingLeft: 16, borderLeft: '3px solid var(--border2)' }}>
                          <div style={{ fontSize: 14, color: 'var(--text)', lineHeight: 1.65 }}>
                            <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--text3)', marginRight: 8 }}>{di + 1}.</span>
                            {d.definition}
                          </div>
                          {d.example && (
                          ...
                          {d.synonyms?.length > 0 && (
                            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6 }}>
                              {d.synonyms.map(s => (
                                <span key={s} onClick={() => { setWord(s); setTimeout(() => inputRef.current?.focus(), 100); }}
                                  style={{ fontSize: 11, padding: '2px 9px', borderRadius: 20, background: 'rgba(99,102,241,0.1)', color: 'var(--primary-light)', border: '1px solid rgba(99,102,241,0.2)', cursor: 'pointer', fontWeight: 600 }}>
                                  {s}
                                </span>
                              ))}
                            </div>
...
export default function StudyTools() {
  ...
  return (
    <div className="animate-fade-up">
      ...
      {/* Tab Bar */}
      <div style={{ display: 'flex', gap: 4, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: 4, width: 'fit-content', marginBottom: 28 }}>
        {TABS.map(t => (
          <motion.button key={t.key} onClick={() => setActiveTab(t.key)}
            whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            style={{
              padding: '9px 20px', borderRadius: 12, fontSize: 13, fontFamily: 'inherit',
              fontWeight: activeTab === t.key ? 700 : 500,
              background: activeTab === t.key ? 'var(--surface3)' : 'transparent',
              color: activeTab === t.key ? 'var(--text)' : 'var(--text3)',
              border: activeTab === t.key ? '1px solid var(--border2)' : '1px solid transparent',
