...
export default function StudentProfile() {
  ...
  const student = {
    ...
    recentActivity: [
      ...
      { id: 2, type: 'attendance', title: 'Live Session: Limits', date: '2026-04-09T09:00:00Z', score: 'Present' },
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 24px' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ background: 'linear-gradient(135deg, var(--surface) 0%, var(--surface2) 100%)', border: '1px solid var(--border)', borderRadius: '32px', padding: '40px', display: 'flex', alignItems: 'center', gap: '32px', marginBottom: '32px', boxShadow: '0 10px 40px rgba(0,0,0,0.02)' }}>
         ...
         <div style={{ flex: 1 }}>
           ...
           <div style={{ display: 'flex', gap: '16px', color: 'var(--text2)', fontSize: '14px', fontWeight: 600 }}>
             ...
             <span>📞 Guardian: {student.guardianContact}</span>
           </div>
         </div>
         <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginBottom: '32px' }}>
        <motion.div {...statCardProps} transition={{ delay: 0.1 }}>
          ...
          <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase' }}>Avg Grade</div>
        </motion.div>
        <motion.div {...statCardProps} transition={{ delay: 0.15 }}>
          ...
          <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase' }}>Assignments</div>
        </motion.div>
        <motion.div {...statCardProps} transition={{ delay: 0.2 }}>
          ...
          <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase' }}>Attendance</div>
        </motion.div>
        <motion.div {...statCardProps} transition={{ delay: 0.25 }}>
          ...
          <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase' }}>Total XP</div>
        </motion.div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '32px' }}>
        ...
        <motion.div {...statCardProps} transition={{ delay: 0.3 }} style={{ ...statCardProps.style, padding: '32px' }}>
          <h3 style={{ fontSize: '18px', fontWeight: 800, color: 'var(--text)', marginBottom: '24px' }}>Recent Activity</h3>
        ...
        <motion.div {...statCardProps} transition={{ delay: 0.4 }} style={{ ...statCardProps.style, padding: '32px', background: 'var(--surface2)' }}>
          <h3 style={{ fontSize: '16px', fontWeight: 800, color: 'var(--text)', marginBottom: '16px' }}>Private Teacher Notes</h3>
          <textarea 
            placeholder="Add a private note about this student's progress or behavior..."
            rows={8}
